...
export function getPropertiesMetadata<T>(cls: IndexedClass<T>): IPropertiesMetadata {
    let properties: IPropertiesMetadata = Reflect.getMetadata(DECORATORS.PROPERTIES, cls);
    if (cls.prototype) {
        let proto = cls.prototype

        do {
            let protoProps = Reflect.getMetadata(DECORATORS.PROPERTIES, proto)
            properties = { ...properties, ...protoProps }
            proto = proto && proto.prototype
        } while (proto && proto.prototype && proto.constructor)  
    }
  if (!properties) {
